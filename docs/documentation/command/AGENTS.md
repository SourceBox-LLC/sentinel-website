...
- `DATABASE_URL` — defaults to `sqlite:///./sentinel.db`. **One codebase, two engines**, selected by this URL's scheme (see `app/core/database.py`, which branches on it):
  - **Hosted production runs Postgres** (`postgresql+psycopg://…`) on the shared `sentinel-postgres` cluster — one cluster, one database per service (`sentinel_command`, `sentinel_license`, `sentinel_sync`). Set as a Fly *secret*, not in `fly.toml`, because it carries a password. **The roles are hardened**, which is not Fly's default: `fly postgres attach` creates every role `SUPERUSER`, which would let any one service's credential read and write the others' databases. Each database is instead owned by its own role, `CONNECT` is revoked from `PUBLIC`, and the roles are `NOSUPERUSER` — verified: all six cross-database connection attempts are refused while each service still reaches its own. **Any future `fly postgres attach` recreates a superuser role and must be re-hardened** (see `docs/runbooks/DISASTER_RECOVERY.md`). Default QueuePool with `pool_pre_ping=True` — `NullPool` would mean a fresh TCP+auth round trip per request now that the database is across a network. Note the `+psycopg` driver suffix: `fly postgres attach` emits a bare `postgres://` URL, which SQLAlchemy would route to psycopg2 (not installed).
  - **Self-hosted runs SQLite** — NullPool, WAL, `synchronous=NORMAL`, `check_same_thread=False`. These are pysqlite-only: the PRAGMA handler is registered *conditionally* and the connect args are applied only for SQLite, because `PRAGMA` is a syntax error on Postgres and psycopg rejects those kwargs. Since the hosted deployment moved to Postgres, this path is now **exclusively** self-hosted — it has no production traffic behind it any more, so treat changes here as changes to someone else's install.
    - `connect_args` sets pysqlite's `timeout=30` and the connect-time PRAGMA sets `busy_timeout=30000` — these **must stay in agreement**, because the PRAGMA runs right after connect and silently overrides the connect arg. They disagreed until 2026-09-07 (30s vs 5s, with 5s winning). 30s rather than fail-fast is deliberate: the hot path (`push_segment`, 1200/min) does no writes and WAL never blocks readers, so the only real contention is writer-vs-writer between background loops (log cleanup's bulk deletes, offline sweep, viewer-usage flush) — where a short timeout turns a heartbeat or motion-event insert into a lost row rather than a slow one. Sentinel-License-Service resolves the same pair identically.
...
- **One fixed admin, one fixed org** (`LOCAL_ORG_ID`, default `"self-host"`) — no
  ...
  just works.
- **Plan**: `resolve_org_plan()`/`effective_plan_for_caps()` in `core/plans.py`
